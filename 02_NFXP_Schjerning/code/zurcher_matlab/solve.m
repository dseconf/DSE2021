classdef solve
	methods (Static)
		...
		function ap=setup(apopt)
		  ...
		  ap.sa_tol=			1.0e-10;			 % Absolute tolerance before (in solve.poly: tolerance before switching to N-K algorithm)
		...
		function [V, P, dV, iter]=poly(bellman, V0, ap, bet)
			% solve.poly: Solve for fixed point using a combination of Succesive Approximations (SA) and Newton-Kantorovich (NK) iterations 
			%
			%  syntax:	[V, P, dV, iter]=solve.poly(bellman, V0, ap, bet)
			%
			%  INPUT:
			...
			%								[m x 1 matrix]
			%
			%			ap:				Algorithm paramaters. See solve.setup
			%
			...
			for k=1:ap.max_fxpiter; %poly-algorithm loop (switching between SA and N-K and back)
			    ...
			    if nargin>3
						[V0, iter(k).sa]=solve.sa(bellman, V0, ap, bet);			    	
			    else
						[V0, iter(k).sa]=solve.sa(bellman, V0, ap);
					end
			    ...
			    [V0, P, dV, iter(k).nk]=solve.nk(bellman, V0, ap);	
		...
		function [V, iter]=sa(bellman, V0, ap, bet)
			% solve.sa: Solve for fixed point using successive approximations
			%
			%  syntax:	[V, P, dV, iter] = solve.sa(bellman, V0, ap, bet):
			%
			%  INPUT:
		...
		function [V, P, dV, iter]=nk(bellman, V0, ap)
			% solve.nk: Solve for fixed point using Newton-Kantorovich iterations 
			%
			%  syntax:	[V, P, dV, iter]=solve.nk(bellman, V0, ap):
			%
			%  INPUT:
			%     bellman:  [V, P, dV]=Bellman equation with fixed point, V
			%	 							Matlab function on the form [V,dV]=bellman(V)
