...
[ev,pk,~,~]=solve.poly(bellman, V0, ap, mp.beta); 
...
elseif ~updateWeightingMatrix
  ...
  [~,mom,mom_indiv]=msm.objective(P, data_moments, W, mp, ap, {'c','RC'}, [est1c, est1RC]');
...
if ~useSolutionForSims
  ...
  [~,sims_moments]=msm.objective(P, data_moments, W, mp, ap, {'c','RC'}, [mp.c, mp.RC]');
...
for i=1:nPicGrid;
  ...
  for j=1:nPicGrid;
    ...
    msmsurf(j,i)=msm.objective(P, data_moments, W, mp, ap, pnames, theta);
    fprintf('.');
  end
  fprintf('\n');  
end
