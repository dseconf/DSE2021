...
mp=zurcher.setup;
nc = 2; 	      	% nc: number of preference parameters to be estimated (e.g. mp.RC and mp.c)
np= numel(mp.p);  	% np: number of free transition matrix parameters to be estimated 
N =mp.n;  			% N: number of EV parameters (i.e. number of grid points) 
M= np+1; 			% M: number transition matrix parameters. 

% ************************************************************************
% INSPECT SPARSITY PATTERNS 
% Jacobian of constraints and Hessian of Lagrangian for likelihood
% ************************************************************************
[J_pattern, H_pattern] = mpec.sparsity_pattern(nc,np,N, M);

fprintf('Number of cost function parameters to be estimated:           nc=%g\n', nc);
fprintf('Number of free transition matrix parameters to be estimated   np=%g\n', np);
fprintf('Number of EV parameters (i.e. number of gridpoints)           N =%g\n', N);
fprintf('Number transition matrix parameters                           M =%g\n', M);
fprintf('Total number variables in constrained optimization problem:   nc+np+N =%g\n', nc+np+N);
...
[V, pk, dV, iterinfo]=solve.poly(@(ev) zurcher.bellman_integrated(ev, mp, P), 0, ap, mp.beta);	

%solve for fixed point in expected value function space
[ev, pk, dev, iterinfo]=solve.poly(@(ev) zurcher.bellman_ev(ev, mp, P), 0, ap, mp.beta);	
