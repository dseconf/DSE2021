classdef mpec
  ...
  methods (Static)
    function [results, pnames, theta_hat, Avar] = estim(data, mp, twostep)
      ...
      nc = 2; % number of structural parameters to be estimated
      ...
      % starting values 
      theta_start_mpec = [struct2vec(mp,pnames); zeros(mp.n,1)];
    ...
    function [c,ceq,DC,DCeq] = con_bellman(data, mp, theta)
      ...
      if nargout >= 3
        [ev1, pk, dev]=zurcher.bellman_ev(ev, mp, P);
      else
        [ev1, pk]=zurcher.bellman_ev(ev, mp, P);
      end
